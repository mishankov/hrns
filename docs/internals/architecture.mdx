...
6. choose interactive mode by default, or `exec` mode when the first CLI argument is `exec`
...
In `exec` mode, the same startup path is reused, but the app creates a fresh message list with the system prompt plus one user message from `-message`, runs the loop once, and exits.
