...
2. reads `HRNS_KEY`, `HRNS_BASE_URL`, and `HRNS_SKIP_VERIFY`
3. builds `openai.Client`
4. loads skills from the default global and local roots
5. creates the `load_skill` tool from the discovered skills
6. builds a hardcoded system prompt, optionally listing discovered skills
7. creates `loop.Loop` with the built-in tools
8. starts the TUI
...
3. `RunLoop` prepends the system message
4. `RunLoop` converts registered tools into OpenAI-style function schemas
5. `openai.Client.StreamChatCompletion` streams SSE events from `/chat/completions`
6. `loop` emits chunks for assistant text, reasoning, and tool call events
7. if the model called tools, `loop` executes them and appends `tool` messages
8. the loop repeats until a streamed response finishes without tool calls
9. the TUI updates its in-memory conversation from `agent.Messages()`
...
The TUI is where model switching and conversation reset happen today.
...
- default model is hardcoded in `tui/tui.go`
