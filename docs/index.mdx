...
- The TUI defaults to the hardcoded model `kimi-k2.6`.
- The app builds an OpenAI-compatible client from environment variables.
...
This documentation describes current behavior only. It does not document items from `TODO.md` as if they already exist.
...
<CardGroup cols={2}>
  ...
  <Card title="Provider setup" icon="plug" href="/guides/provider-setup">
    Point hrns at any OpenAI-compatible endpoint with the supported environment variables.
  </Card>
...
1. `main.go` builds a client, loads skills, assembles the tool map, and starts the TUI.
2. The TUI collects user input and sends the conversation to `loop.RunLoop`.
